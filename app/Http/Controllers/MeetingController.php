 ...
 * app/UseCases/Meeting の各 Action(1 メソッド = 1 Action)に委ね、Controller はリクエスト受付・
 * 認可($this->authorize() / FormRequest::authorize())・レスポンス整形のみを担う。
 */
class MeetingController extends Controller
{
    ...
    {
        $result = $action($request->user(), $request->validated('filter'));
    ...
    {
        $studentId = $request->validated('student');
        $enrollmentId = $request->validated('enrollment');

        $result = $action($request->user(), $request->validated('filter'), $studentId, $enrollmentId);
    ...
    /**
     * 受講生の予約申請。残面談回数の確認・コーチ自動割当・面談回数消費・通知は StoreAction が担う。
     */
    ...
    /**
     * 担当コーチによる面談メモ作成・更新。canceled の面談にはメモを残せない(UpsertMemoAction で検証)。
     */
